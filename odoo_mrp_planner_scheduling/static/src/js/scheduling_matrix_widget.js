...
class SchedulingMatrixWidget extends Component {
    ...
    _computeRows() {
        ...
        for (const row of this.state.rows) {
            ...
            // Filtrar barras por búsqueda — NUNCA en modo ruta: la ruta es una
            // cadena específica (enfocada + padres/hijas). El buscador está oculto
            // en ruta pero su valor persiste, y aplicarlo descartaba las barras de
            // las OFs relacionadas (distinto mo_name que la enfocada) → filas vacías.
            let bars = this.state.routeMode ? (row.bars || []) : (row.bars || []).filter(matchBar);
