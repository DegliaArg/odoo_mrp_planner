...
El widget de **demanda insatisfecha** (`unmet_demand_widget.js` / `.xml`) usa un archivo único: dos datasets independientes (gráfico vs cards+tabla), KPIs, gráfico Chart.js y tabla con `useColManager` (columnas reordenables/redimensionables). En la sección inferior las cards se renderizan **arriba** de la barra de filtros. Los **filtros cruzados** (cliente/producto/familia) no viven en el widget sino en el `PlannerSearchBar` compartido (props `crossDefs`/`crossValues`/`onCrossSet`/`onCrossRemove`, sección "Cruzar con" con `AutoComplete` + `name_search`, chips tipo `cross`), reutilizable por cualquier panel; el widget solo mapea sus dimensiones cruzables y pasa el `cross_filters` al RPC. El **gráfico de ventas** (`sales_chart_widget`) dibuja el ranking de barras más dos donas (categoría ABC y familia de producto).
...
| `mrp_planner_dashboard_unmet.py` | Análisis de demanda insatisfecha: `get_unmet_demand_data(period, dimension, wh, amount_method, include_all, cross_filters)` (backlog pendiente = pedido − entregado por cliente/producto/familia, antigüedad del pendiente, días quiebre vía `_stock_break_days_map` y **situación** de entregabilidad histórica; `include_all` = toggle "solo faltantes"; `cross_filters` = filtros cruzados cliente/producto/familia que recortan las líneas), `get_unmet_family_universe(period)` (familias presentes en el período, para acotar el autocompletar del cruce por familia al universo de análisis), `action_open_unmet_demand` y `action_open_unmet_lines`. Comparte base con customer (mismos estados de pedido, filtro empresa/almacén, exclusión de servicios y PxQ/Real). |
