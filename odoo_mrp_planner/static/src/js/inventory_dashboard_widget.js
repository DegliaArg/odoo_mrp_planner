...
class InventoryDashboardWidget extends Component {
    ...
    kpiTooltip(key) {
        ...
        switch (key) {
            ...
            case "delivered":
                return `Remitos validados en el rango de fechas de la tabla, por fecha de validación (con sus depósitos)\nSin tipos seleccionados cuenta solo SALIDAS (el eslabón final, sin doble conteo); con tipos seleccionados cuenta los validados de esos tipos — ojo: varios eslabones de la misma cadena duplican la mercadería\nSuma de las cantidades hechas\n→ ${fmt(k.delivered_qty)} Pz en ${k.delivered_pickings || 0} remito(s)${this.selectedRows.length ? "\nLa selección de filas no aplica acá: mide lo YA entregado, que no está en la tabla." : ""}`;
            case "rate":
