...
class MrpPlannerDashboardWc(models.TransientModel):
    ...
    def get_wc_load_data(self):
        """Devuelve el ranking de los 10 CTs con mayor carga planificada en OFs activas.

        Suma las horas esperadas (``duration_expected`` en minutos, convertidas a horas)
        de todos los workorders en estados activos cuya OF esté también en curso.
        Excluye operaciones de subcontratación para no distorsionar la carga interna.

        El porcentaje de barra (``bar_pct``) se calcula relativo al CT con mayor carga,
        lo que permite usar el resultado directamente como datos para un gráfico de barras
        horizontales normalizadas.

        :returns: list[dict] — hasta 10 entradas ordenadas de mayor a menor carga, cada
                  una con las claves ``id`` (int), ``name`` (str), ``hours`` (float,
                  redondeado a 1 decimal) y ``bar_pct`` (int, 0-100).
                  Devuelve lista vacía si no hay workorders activos.
        """
        domain = [
        ...
        rows = sorted(wc_map.values(), key=lambda x: -x['hours'])[:10]  # top-10 para no saturar el widget
        if not rows:
            return []
        max_h = rows[0]['hours'] or 1.0  # fallback a 1.0 para evitar división por cero si el mayor es 0
        for r in rows:
